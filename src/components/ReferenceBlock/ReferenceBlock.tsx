  ...
  return (
    ...
    >
      <Image
        ...
        priority
        className={
          "mr-4 " + (props.isSimple === true ? "w-12 h-12" : "w-14 h-14")
        }
      ></Image>
      <div
        className={`flex-grow w-32 ${props.isSimple === true ? "h-8" : "h-12"}`}
      >
        <div className="flex flex-col items-start">
          <div className="flex">
            {props.moreOption?.map((option, idx) => {
              return (
                <div
                  key={idx}
                  className={`border px-2 mr-1.5 rounded-md border-${
                    option === "new" ? "yellow-600" : "black"
                  }
                                         text-xs text-${
                                           option === "new"
                                             ? "yellow-600"
            ...
            <div
              className={`border px-2 mr-2 rounded-md border-${
          ...
          {props.isSimple === true ? (
            ...
            <RefInfoView
              refdata={refdata}
              className="text-xs text-gray1 truncate ..."
            />
          )}
        </div>
      </div>
      <div className="p-1 mt-auto">
        {props.isDropMenu &&
        props.stateNum !== undefined &&
        props.setStateNum !== undefined ? (
          <StudyDropMenu
            stateNum={props.stateNum}
