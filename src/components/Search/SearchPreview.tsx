...
export const SearchPreview = (props: {
      ...
      searchAllApi({ search: props.searchString }).then((data) => {
        ...
        data.node.length !== 0
          ? setSelected({
              ...
              id: data.node[0].nid,
              type: "node",
              category: data.node[0].type,
              nodeName: data.node[0].name,
            })
          : data.reference.length !== 0
          ? setSelected({
              idx: 0,
              id: data.reference[0].rid,
              type: "reference",
              category: data.reference[0].category,
              nodeName: data.reference[0].node[0].name,
            })
          : null;
      });
    }, 300);
  ...
  return (
    <div className="w-full h-full px-4 py-5">
      {searchResult.node.length > 0 && (
        <div id="nodeSearchView">
          <div id="nodeSearchViewTitleNode" className="text-base">
            분야
          </div>
          <ul
            id="nodeSearchViewListNode"
            role="listbox"
            className="overflow-y-scroll text-sm max-h-36"
          >
            {searchResult.node.map((node, idx) => {
              return (
                <li
                  className={`flex items-center my-2 rounded-lg ${
                    selected?.idx === idx ? "bg-gray-50" : "bg-white"
                  }`}
                  key={"node" + idx}
                  onMouseOver={() => {
                    setSelected({
                ...
                >
                  <Link
                    ...
                    href={`/roadmap/${categorytoNum[node.type]}?node=${
                      node.name
                    }`}
                    onClick={() => {
                      closeModal();
      ...
      {searchResult.node.length !== 0 &&
        searchResult.reference.length !== 0 && <hr className="my-3 mt-4" />}
      {searchResult.reference.length > 0 && (
        <div id="referenceSearchView">
          <div id="nodeSearchViewTitleRef" className="text-base">
            강의자료
          </div>
          <ul
            id="nodeSearchViewListRef"
            role="listbox"
            className="overflow-y-scroll text-sm max-h-64"
          >
              ...
              return (
                <li
                  ...
                  onMouseOver={() => {
                    setSelected({
                      ...
                      id: ref.rid,
                      type: "reference",
                      category: ref.node[0].type,
                      nodeName: ref.node[0].name,
                    });
                  }}
                  role="option"
                  ref={(r) => (searchRef.current[idx] = r)}
                >
                  <Link
                    ...
                    href={`/roadmap/${categorytoNum[ref.node[0].type]}?node=${
                      ref.node[0].name
                    }&ref=${ref.rid}`}
                    onClick={() => {
                      closeModal();
      ...
      {searchResult.node.length === 0 &&
        searchResult.reference.length === 0 && (
          <div className="mx-auto text-sm text-gray-400 w-36">
            검색 결과가 없습니다.
          </div>
        )}
    </div>
