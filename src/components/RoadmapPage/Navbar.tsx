...
export default function Navbar() {
  ...
  return (
    <header className="z-50 py-2 border-b-2 h-16">
      <div className="flex mx-5">
        <Link className="mt-0 grow basis-0" href="/">
          <Logo className="text-lg text-white flex hover:cursor-pointer" />
        </Link>
        <div className="flex lg:mt-0 lg:grow lg:basis-0 lg:justify-end">
          {init ? (
            isLoggedIn === false ? (
              <button
                className="inline-flex justify-center rounded-2xl bg-main m-1.5 p-1.5 text-base font-semibold text-white hover:brightness-95 focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-500 active:text-white/70"
                onClick={() => {
                  console.log('login');
                }}
              >
